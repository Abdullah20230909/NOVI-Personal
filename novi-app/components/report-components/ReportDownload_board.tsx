...
export default function ReportDownloadBoard() {
    ...
    useEffect(() => {
        const fetchSessions = async () => {
            ...
            try {
                ...
                console.error('Failed to fetch teacher download report data:', e);
                setSessions([]);
            }
            setLoading(false);
        };
    ...
    return (
        <div className="flex flex-col flex-1 animate-fade-in w-full">
                    ...
                    <div className="bg-white rounded-3xl p-8 shadow-2xl max-w-[420px] w-full mx-4 border border-cyan-100">
                        ...
                        <div className="flex gap-3 justify-end">
                            <button
                                ...
                                className="px-6 py-2.5 rounded-xl font-bold text-sm text-[#0284c7] bg-[#ecfeff] hover:bg-[#cffafe] transition-colors border border-cyan-200"
                            >
                                Cancel
                            </button>
                            <button
            ...
            <div className="bg-white rounded-[24px] p-8 shadow-sm border-2 border-[#06b6d4] w-full">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-3xl font-black text-[#06b6d4] tracking-tight">Archived Sessions</h2>
                        <p className="text-[#8ba3b8] font-medium text-sm mt-2">
                            Sessions are <span className="text-[#06b6d4] font-semibold">automatically synced</span> and stored here for your review.
                        </p>
                    </div>
                    {sessions.length > 1 && (
                        <button
                            onClick={() => setSortOrder(prev => prev === 'newest' ? 'oldest' : 'newest')}
                            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#ecfeff] hover:bg-[#cffafe] transition-colors text-[#06b6d4] font-bold text-sm border border-cyan-200"
                        >
            ...
            {loading && (
                <div className="mt-6 text-[#0284c7] font-bold animate-pulse text-center py-16">Loading sessions...</div>
            )}
            ...
            {!loading && sortedSessions.length > 0 && (
                <div className="flex flex-col gap-5 mt-6">
                    {sortedSessions.map((session, index) => (
                        <div key={index} className="bg-[#06b6d4] rounded-[20px] p-6 shadow-sm flex items-center justify-between w-full">
                            <div className="flex flex-col gap-3 flex-1">
                                {/* Meeting ID Block */}
                                <div className="bg-white rounded-xl px-5 py-3 max-w-[550px]">
                                    <span className="text-[#06b6d4] font-bold text-sm">Meeting ID: <span className="font-semibold text-[#0284c7]">{session.session_id}</span></span>
                                </div>
                                {/* Date & Time Metadata Blocks */}
                                <div className="flex gap-3">
                                    <div className="bg-white rounded-xl px-5 py-2.5 min-w-[180px]">
                                        <span className="text-[#06b6d4] font-bold text-sm">Generated Date: <span className="font-semibold text-[#0284c7]">{session.generated_date}</span></span>
                                    </div>
                                    <div className="bg-white rounded-xl px-5 py-2.5 min-w-[180px]">
                                        <span className="text-[#06b6d4] font-bold text-sm">Generated Time: <span className="font-semibold text-[#0284c7]">{session.generated_time}</span></span>
                                    </div>
                                </div>
                            </div>
            ...
            {!loading && sessions.length === 0 && (
                <div className="mt-6 bg-[#ecfeff] rounded-[24px] p-8 shadow-sm border-2 border-dashed border-cyan-200 flex flex-col items-center justify-center min-h-[250px] w-full">
                    ...
                    <p className="text-[#0284c7]/60 font-medium text-sm italic text-center">
